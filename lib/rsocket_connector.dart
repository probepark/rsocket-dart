...
class _ReconnectingRSocket extends RSocket {
  ...
  _ReconnectingRSocket(this._connector, String url) {
    requestResponse = (payload) async {
      final active = _activeRSocket;
      if (active == null)
        throw RSocketException(
            RSocketErrorCode.CONNECTION_ERROR, 'No active connection');
...
class RSocketConnector {
  ...
  final BehaviorSubject<ConnectionEvent> _connectionStateController =
      BehaviorSubject<ConnectionEvent>.seeded(
  ...
  final BehaviorSubject<ConnectionHealth> _healthController =
      BehaviorSubject<ConnectionHealth>.seeded(
          ConnectionHealth(isHealthy: false, lastHeartbeat: DateTime.now()));

  Stream<ConnectionEvent> get connectionStateStream =>
  ...
  Future<RSocket> _connectOnce(String url) async {
    ...
    try {
      ...
      final rsocketRequester = RSocketRequester(
          'requester', connectionSetupPayload, conn,
      ...
      if (_acceptor != null) {
        ...
        if (rsocketRequester.responder == null) {
          ...
          throw RSocketException(RSocketErrorCode.REJECTED_SETUP,
              'Connection refused, please check setup and security!');
        }
      } else {
      ...
      _connectionStateController
          .add(ConnectionEvent(ConnectionState.connected));
      if (!_healthController.isClosed) {
      ...
      _connectionStateController
          .add(ConnectionEvent(ConnectionState.failed, error: error));
      rethrow;
    }
  }
