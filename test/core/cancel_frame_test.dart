...
void main() {
  group('CANCEL Frame Tests', () {
    ...
    test('Should send CANCEL frame when stream is cancelled', () async {
      ...
      var stream = requester.requestStream!(
          Payload()..data = Uint8List.fromList('test'.codeUnits));
    ...
    test('Should clean up all streams when connection closes', () async {
      ...
      expect(
          streamCompleter1.future,
          throwsA(isA<RSocketException>().having(
              (e) => e.code, 'code', RSocketErrorCode.CONNECTION_CLOSE)));
      expect(
          streamCompleter2.future,
          throwsA(isA<RSocketException>().having(
              (e) => e.code, 'code', RSocketErrorCode.CONNECTION_CLOSE)));
    ...
    test('Should ignore CANCEL frame for non-existent stream', () {
      ...
      // Should not throw
      expect(
          () => connection.simulateIncomingFrame(cancelFrame), returnsNormally);
    });
    ...
    test('CompleterSubscriber handles cancel error', () async {
      ...
      // Simulate cancel
      subscriber.onError(
          RSocketException(RSocketErrorCode.CANCELED, 'Request cancelled'));
    ...
    test('StreamSubscriber completes stream on cancel', () async {
      ...
      // Send some data
      subscriber
          .onNext(Payload()..data = Uint8List.fromList('data1'.codeUnits));
      subscriber
          .onNext(Payload()..data = Uint8List.fromList('data2'.codeUnits));
